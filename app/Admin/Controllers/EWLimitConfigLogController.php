...
use App\Models\EWLimitConfigLog;
...
class EWLimitConfigLogController extends AdminController
{
	/**
	 ...
	 * @var string
	 */
	protected $title = "EWLimitConfigLog";
	...
	protected function form()
	{
		...
		$form = new Form(new EWLimitConfigLog);

		$ew_limit_config_log_id = (request()->route()->parameter("ew_limit_config_log")) ? request()->route()->parameter("ew_limit_config_log") : "";
		if($ew_limit_config_log_id){
			$ew_limit_config_loginfo = EWLimitConfigLog::find($ew_limit_config_log_id);
		}
		//$form->display("id", "ID");
		$ew_limit_config_id=$form->number("ew_limit_config_id", __("Limit config id"))->required();
        $previous_data=$form->text("previous_data", __("Previous Data"))->required();
        $current_data=$form->text("current_data", __("Current Data"))->required();
		...
		$productFormSettings = ProductForm::where("model_name", "EWLimitConfigLog")->first();
	...
	protected function grid()
	{
		...
		$grid = new Grid(new EWLimitConfigLog);

		$grid->column("id", __("Id"));
		if (empty($enableFiledList)) {
			$grid->column("ew_limit_config_id", __("Limit config id"))->display(function ($ew_limit_config_id) {
        	return empty($ew_limit_config_id) ? "" : $ew_limit_config_id;
        });
        $grid->column("previous_data", __("Previous Data"))->display(function ($previous_data) {
        	return empty($previous_data) ? "" : $previous_data;
        });
        $grid->column("current_data", __("Current Data"))->display(function ($current_data) {
        	return empty($current_data) ? "" : $current_data;
        });
		} else {
		...
		$grid->filter(function($filter){
			...
			$filter->disableIdFilter();
			            $filter->like("ew_limit_config_id", __("Limit config id"));
            $filter->like("previous_data", __("Previous Data"));
            $filter->like("current_data", __("Current Data"));
    ...
    {
        $show = new Show(EWLimitConfigLog::findOrFail($id));

		//$show->id("ID");
		$show->field("ew_limit_config_id", __("Limit config id"));
        $show->field("previous_data", __("Previous Data"));
        $show->field("current_data", __("Current Data"));
