...
class EWOTPHistoryController extends AdminController
{
	/**
	...
	protected function form()
	{
		...
		$form = new Form(new EWOTPHistory);

		$ew_otp_history_id = (request()->route()->parameter("ew_otp_history")) ? request()->route()->parameter("ew_otp_history") : "";
		if($ew_otp_history_id){
			$ew_otp_historyinfo = EWOTPHistory::find($ew_otp_history_id);
		}
		//$form->display("id", "ID");
		        $mobile_no=$form->text("mobile_no", __("Mobile No."))->required();
        $otp=$form->text("otp", __("OTP"))->required();
        $otp_used=$form->select("otp_used", __("OTP used"))->options(['yes'=>'yes','no'=>'no']);
        $smsgw_response=$form->text("smsgw_response", __("SMSGW Response"));
        $channel=$form->select("channel", __("Channel"))->options(['register'=>'register','forgot'=>'forgot']);
		...
		$productFormSettings = ProductForm::where("model_name", "EWOTPHistory")->first();
	...
	protected function grid()
	{
		...
		$grid = new Grid(new EWOTPHistory);
        ...
        $grid->column("otp_used", __("OTP used"));
        $grid->column("smsgw_response", __("SMSGW Response"))->display(function ($smsgw_response) {
        	return empty($smsgw_response) ? "" : $smsgw_response;
        });
		...
		$grid->filter(function($filter){
            ...
            $filter->equal("otp_used", __("OTP used"))->select(['yes'=>'yes','no'=>'no']);
            $filter->equal("channel", __("Channel"))->select(['register'=>'register','forgot'=>'forgot']);
    ...
    {
        $show = new Show(EWOTPHistory::findOrFail($id));

		//$show->id("ID");
		$show->field("mobile_no", __("Mobile No."));
        $show->field("otp", __("OTP"));
        $show->field("otp_used", __("OTP used"));
        $show->field("smsgw_response", __("SMSGW Response"));
