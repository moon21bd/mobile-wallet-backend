...
class EWCityController extends AdminController
{
	/**
	...
	protected function form()
	{
		...
		$form = new Form(new EWCity);

		$ew_city_id = (request()->route()->parameter("ew_city")) ? request()->route()->parameter("ew_city") : "";
		if($ew_city_id){
			$ew_cityinfo = EWCity::find($ew_city_id);
		}
		//$form->display("id", "ID");
		...
		$productFormSettings = ProductForm::where("model_name", "EWCity")->first();
	...
	protected function grid()
	{
		...
		$grid->filter(function($filter){
            ...
            if (Admin::user()->organization_id != 1) {
            	...
            	$options_dd_350 = $dropdownqry->pluck("name", "id");
            }else{
            	$options_dd_350 = EWCountry::pluck("name", "id");
            }
            $filter->equal("country_id", __("Country"))->select($options_dd_350);
    ...
    {
        $show = new Show(EWCity::findOrFail($id));
