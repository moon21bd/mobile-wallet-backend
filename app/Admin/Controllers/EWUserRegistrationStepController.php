...
use App\Models\EWUserRegistrationStep;
...
class EWUserRegistrationStepController extends AdminController
{
	/**
	 ...
	 * @var string
	 */
	protected $title = "EWUserRegistrationStep";
	...
	protected function form()
	{
		...
		$form = new Form(new EWUserRegistrationStep);

		$ew_user_registration_step_id = (request()->route()->parameter("ew_user_registration_step")) ? request()->route()->parameter("ew_user_registration_step") : "";
		if($ew_user_registration_step_id){
			$ew_user_registration_stepinfo = EWUserRegistrationStep::find($ew_user_registration_step_id);
		}
		//$form->display("id", "ID");
		        $step=$form->text("step", __("Step"))->required();
        $user_id=$form->number("user_id", __("User"))->required();
		...
		$productFormSettings = ProductForm::where("model_name", "EWUserRegistrationStep")->first();
	...
	protected function grid()
	{
		...
		$grid = new Grid(new EWUserRegistrationStep);
		...
		$grid->filter(function($filter){
			...
			$filter->like("step", __("Step"));
            $filter->like("user_id", __("User"));
    ...
    {
        $show = new Show(EWUserRegistrationStep::findOrFail($id));
	...
	public function getUseTableName(){
		return 'ew_user_registration_steps';
	}
