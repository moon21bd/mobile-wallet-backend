...
class EWUserPendingDetailController extends AdminController
{
	/**
	 ...
	 * @var string
	 */
	protected $title = "User Pending Detail";
	...
	protected function form()
	{
		...
		$ew_user_detail_id = (request()->route()->parameter("ew_user_pending_detail")) ? request()->route()->parameter("ew_user_pending_detail") : "";
	...
	protected function grid()
	{
		...
		$grid->filter(function($filter){
            ...
            if (Admin::user()->organization_id != 1) {
            	...
            	$options_dd_345 = $dropdownqry->pluck("name", "id");
            }else{
            	$options_dd_345 = EWCountry::pluck("name", "id");
            }
            $filter->equal("country_id", __("Country"))->select($options_dd_345);
            if (Admin::user()->organization_id != 1) {
            	$dropdownqry = EWCity::select("name", "id");
            	if (Admin::user()->account_type == "system") {
            		$dropdownqry->where("organization_ref_id", "like", Admin::user()->organization_ref_id."%");
            	...
            	$options_dd_346 = $dropdownqry->pluck("name", "id");
            }else{
            	$options_dd_346 = EWCity::pluck("name", "id");
            }
            $filter->equal("city_id", __("City"))->select($options_dd_346);
    ...
    protected function detail($id)
    {
        ...
        $show->field("id_card_image", __("NID Front Image"))->image();
        $show->field("selfie_image", __("Selfie Image"))->image();
        $show->field("status", __("Status"));
        $show->field("comment", __("Comment"));
        $show->field("otp", __("OTP"));
        $show->field("ewcountry_country_id.name", __("Country"));
        $show->field("ewcity_city_id.name", __("City"));
	...
	public function getUseTableName(){
		return 'ew_user_details,ew_countries,ew_cities';
	}
