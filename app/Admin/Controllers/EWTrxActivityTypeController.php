...
class EWTrxActivityTypeController extends AdminController
{
	/**
	 ...
	 * @var string
	 */
	protected $title = "EWTrxActivityType";
	...
	protected function form()
	{
		...
		$form = new Form(new EWTrxActivityType);

		$ew_trx_activity_type_id = (request()->route()->parameter("ew_trx_activity_type")) ? request()->route()->parameter("ew_trx_activity_type") : "";
		if($ew_trx_activity_type_id){
			$ew_trx_activity_typeinfo = EWTrxActivityType::find($ew_trx_activity_type_id);
		}
		//$form->display("id", "ID");
		        $activity_type=$form->text("activity_type", __("Activity Type"))->required();
        $slug=$form->text("slug", __("Slug"))->required();
		...
		$productFormSettings = ProductForm::where("model_name", "EWTrxActivityType")->first();
	...
	protected function grid()
	{
		...
		$grid = new Grid(new EWTrxActivityType);

		$grid->column("id", __("Id"));
		if (empty($enableFiledList)) {
			$grid->column("activity_type", __("Activity Type"))->display(function ($activity_type) {
        	return empty($activity_type) ? "" : $activity_type;
        });
		...
		$grid->filter(function($filter){
			...
			$filter->like("activity_type", __("Activity Type"));
    ...
    {
        $show = new Show(EWTrxActivityType::findOrFail($id));
