...
use App\Admin\Controllers\EWLimitConfigController;
...
class EWLimitConfigChildController extends EWLimitConfigController
{
    /**
    ...
    protected function form()
    {
        ...
        $form = new Form(new EWLimitConfig);

        $ew_limit_config_id = (request()->route()->parameter("ew_limit_config")) ? request()->route()->parameter("ew_limit_config") : "";
        if ($ew_limit_config_id) {
            $ew_limit_configinfo = EWLimitConfig::find($ew_limit_config_id);
        }
        //$form->display("id", "ID");
        $trx_type = $form->number("trx_type", __("Transaction Type"))->required();
        ...
        $productFormSettings = \App\Models\Platform\ProductForm::where("model_name", "EWLimitConfig")->first();
    ...
    protected function grid()
    {
        ...
        $grid = new Grid(new EWLimitConfig);

        $grid->column("id", __("Id"));
        if (empty($enableFiledList)) {
            $grid->column("trx_type", __("Transaction Type"))->display(function ($trx_type) {
                return empty($trx_type) ? "" : $trx_type;
            });
        ...
        $productFormSettings = ProductForm::where("model_name", "EWLimitConfig")->first();
        ...
        $grid->filter(function ($filter) {
            ...
            $filter->like("trx_type", __("Transaction Type"));
    ...
    {
        $show = new Show(EWLimitConfig::findOrFail($id));
